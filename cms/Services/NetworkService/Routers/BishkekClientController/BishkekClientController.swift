...
enum BishkekClientController: BaseRouter {
    ...
    var path: String {
        switch self {
        case .getAll:
            return "/bishkek/client"
        case .addClient:
            return "/bishkek/client"
        case .getFile:
            return "/bishkek/client/export"
        case .filter:
            return "/bishkek/client/filter"
        case .importFile:
            return "/bishkek/client/import"
        case .search:
            return "/bishkek/client/search"
        case let .getAllByStatus(status_id):
            return "/bishkek/client/status/\(status_id)"
        case let .changeCity(client_id):
            return "/bishkek/client/\(client_id)/city"
        case let .addPayment(client_id, _):
            return "/bishkek/client/\(client_id)/payment"
        case let .deletePayment(client_id, payment_id):
            return "/bishkek/client/\(client_id)/payment/\(payment_id)"
        case let .editPayment(client_id, payment_id, _):
            return "/bishkek/client/\(client_id)/payment/\(payment_id)"
        case let .changeStatus(client_id, status_id):
            return "/bishkek/client/\(client_id)/status/\(status_id)"
        case let .deleteClient(id):
            return "/bishkek/client/\(id)"
        case let .getById(id):
            return "/bishkek/client/\(id)"
        case let .updateClient(id, _):
            return "/bishkek/client/\(id)"
        case .getClientsWithExpiredTimer:
            return "/bishkek/client/timer"
        }
    }
