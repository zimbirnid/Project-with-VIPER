//
//  ClientsPageClientsPageInteractor.swift
//  cms_mobile
//
...
class ClientsPageInteractor {
    weak var output: ClientsPageInteractorOutput!
...
extension ClientsPageInteractor: ClientsPageInteractorInput {
    func filterAgain() {
        filter(
            courses: courses,
            statuses: statuses,
    ...
    func filter(
        ...
        networkService.sendRequest(
            ...
            case .success(let model):
                ...
                DispatchQueue.main.async {
                    output.didGet(clients: data, updateData: true)
                }
    ...
    func getClients(update: Bool) {
        ...
        networkService.sendRequest(
            ...
            case .success(let model):
                guard
                    let data = model.content?.filter({ (m) -> Bool in
                        if let doska = m.status?.doska, !doska,  !["запись на курс", "записаться на курс"].contains(m.status?.name?.lowercased()) {
                            return true
                        }
                        return false
                    })
                ...
                DispatchQueue.main.async {
                    output.didGet(clients: data, updateData: update)
                }
                self.isGettingFromServer = false
                self.currentPage += 1
                self.previousPage += 1
