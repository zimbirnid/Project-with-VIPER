//
//  StudentsPageViewController.swift
//  cms_mobile
//
...
class StudentsPageViewController: BaseViewController {
    ...
    var output: StudentsPageViewOutput!
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        output.getStudents(update: true)
        if isFirstTime {
    ...
    private func configureTopBar() {
        ...
        topBar.setTitle(title: "Студенты")
        output.getUsername()
...
extension StudentsPageViewController: StudentsPageViewInput {
    ...
    func didGet(model: [ClientModel], updateData: Bool) {
        if updateData {
            ...
            allData.append(contentsOf: model)
        }
        removeSpinner()
        if data.count == 0 {
            output.getStudents(update: false)
        }
    }
...
extension StudentsPageViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: StudentCard.identifier(), for: indexPath) as? StudentCard else {return UICollectionViewCell()}
        cell.configure(model: data[indexPath.item])
        if indexPath.row == data.count - 1 {
            isFiltered ? output.filterAgain() : output.getStudents(update: false)
        }
        return cell
    }
    
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        CGSize(width: collectionView.frame.width-16, height: 72)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, insetForSectionAt section: Int) -> UIEdgeInsets {UIEdgeInsets(top: 4, left: 0, bottom: 4, right: 0)}
...
extension StudentsPageViewController: UISearchBarDelegate {
...
extension StudentsPageViewController: LogoutViewDelegate {
    func logoutTapped() {
...
extension StudentsPageViewController: TopBarDelegate {
